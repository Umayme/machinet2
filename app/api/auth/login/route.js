...
export async function POST(request) {
  try {
    ...
    const db = getDb()
    const user = db.prepare('SELECT id, email, password, name, role, approved FROM User WHERE email = ?').get(email)
    db.close()
    ...
    const response = NextResponse.json({ success: true, user: { id: user.id, email: user.email, name: user.name, role: user.role, approved: Boolean(user.approved) } })
